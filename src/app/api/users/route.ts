...
export async function PATCH(request: NextRequest) { try { const actor = await requireUser(request, ['ADMIN', 'SUPERADMIN']); const body: unknown = await request.json(); if (!isRecord(body) || !Number.isInteger(body.id) || typeof body.password !== 'string' || !validatePassword(body.password).isValid) return NextResponse.json({ error: 'Mật khẩu mới không đạt quy tắc.' }, { status: 400 }); const updated = await resetUserPassword(body.id as number, body.password, actor.id); return updated ? NextResponse.json({ success: true }) : NextResponse.json({ error: 'Không tìm thấy user.' }, { status: 404 }); } catch (error) { return errorResponse(error) ?? NextResponse.json({ error: 'Không thể cấp lại mật khẩu.' }, { status: 500 }); } }
